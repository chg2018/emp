...
<script language="javascript" type="text/javascript">
	...
	function save() {
		var empno = $("input[name='empno']").val().trim();
		...
		if (r) {
			$.post("update.do", {
				...
				if (data == "success") {
					...
					$("#save_result_info").text("修改员工" + ename + "信息成功！");
					...
					$("#save_result_info").text("修改员工" + ename + "信息失败！");
...
<body>
	...
	<div id="main" style="margin: 100px auto">
		...
		<form action="" method="" class="main_form form-inline">
			...
			<div class="input_info">
				<input type="text" class="width300" value="${emp.ename}"
					name="ename" readonly="readonly" /> <span class="required">*</span>
			...
			<div class="input_info">
				<select name="deptno" class="width200" style="width: 205px;">
					<option value="-1" <c:if test="${emp.deptno == '-1'}">selected</c:if>>请选择</option>
						<option value="10" <c:if test="${emp.deptno == '10'}">selected</c:if>>系统集成部</option>
						<option value="20" <c:if test="${emp.deptno == '20'}">selected</c:if>>产品研发部</option>
						<option value="30" <c:if test="${emp.deptno == '30'}">selected</c:if>>市场部</option>
						<option value="40" <c:if test="${emp.deptno == '40'}">selected</c:if>>人力资源部</option>
						<option value="50" <c:if test="${emp.deptno == '50'}">selected</c:if>>其它</option>
				</select> <span class="required">*</span>
				<div class="validate_msg_short"></div>
			</div>
